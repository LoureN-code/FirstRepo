...
def dialog_user_info_to_str(user_data) -> str:
    ...
    mapper = {'language_from': 'Язык оригинала', 'language_to': 'Язык перевода',
              'text_to_translate': 'Текст для перевода', "name": "Имя", "experience": "Опыт работы",
              "additional information": "Дополнительная информация", "skills": "Навыки","exit": "Выход"}
    for key, name in mapper.items():
        if key in user_data:
            result += name + ": " + user_data[key] + "\n"
    return result
